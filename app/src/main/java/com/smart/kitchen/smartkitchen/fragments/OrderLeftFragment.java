...
import com.smart.kitchen.smartkitchen.adapters.LeftOrderAdapter;
...
public class OrderLeftFragment extends BaseFragment implements OnRefreshListener, OnClickListener, OrderLeftView, OnLoadListener {
    private LeftOrderAdapter adapter;
    ...
    protected void initEvent() {
        ...
        this.listView.setOnItemClickListener(new OnItemClickListener() {
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long j) {
                OrderLeftFragment.this.adapter.setSelect(i);
                OrderLeftFragment.this.selectOrderinfo = (OrderInfo) OrderLeftFragment.this.list.get(i);
                OrderLeftFragment.this.selectIndex = i;
                if (OrderLeftFragment.this.mListener != null) {
                    OrderLeftFragment.this.mListener.dataTransmission(JSON.toJSONString(OrderLeftFragment.this.list.get(i)), OrderLeftFragment.this.select);
                }
            }
        });
        this.listView.setOnScrollListener(new OnScrollListener() {
            ...
            public void onScroll(AbsListView absListView, int i, int i2, int i3) {
                OrderLeftFragment.this.seTndex = i;
            }
        });
        this.dialogUtils.screenListener(new ScreenListener() {
            public void type(String str, String str2, String str3, String str4, String str5) {
                if (TextUtils.isEmpty(str3) || TextUtils.isEmpty(str5)) {
                    Collection notifyList = new orderScreenUtils().notifyList(OrderLeftFragment.this.list, str);
                    OrderLeftFragment.this.list.clear();
                    OrderLeftFragment.this.list.addAll(notifyList);
                    OrderLeftFragment.this.adapter.notifyDataSetChanged();
                } else {
                    OrderLeftFragment.this.mSwipeLayout.setRefreshing(true);
                    OrderLeftFragment.this.prestener.getFromNet(str2 + " " + str3, str4 + " " + str5, 1);
                }
                OrderLeftFragment.this.setSelectIndex();
            }
        });
    }
    ...
    public void setSelectIndex() {
        ...
        if (index != -1) {
            ...
            if (this.mListener != null) {
                this.mListener.dataTransmission("notify", this.select);
            }
        } else if (this.selectIndex >= this.list.size()) {
            this.adapter.setSelect(this.list.size() - 1);
            this.listView.setSelection(this.seTndex);
            if (this.mListener != null) {
                this.mListener.dataTransmission(JSON.toJSONString(this.list.get(this.list.size() - 1)), this.select);
            }
        } else {
    ...
    public void onRefresh() {
        ...
        new Handler().postDelayed(new Runnable() {
            public void run() {
                OrderLeftFragment.this.prestener.getFromNet(null, null, 1);
            }
        }, 2000);
    }
    ...
    private void initViewList() {
        ...
        if (this.adapter == null) {
            this.adapter = new LeftOrderAdapter(this.context, this.list);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.ll_shaixuan:
                this.dialogUtils.showShaiXuan(view, "order" + this.select);
                return;
            case R.id.tv_tangchi:
                ...
                ((OrderActivity) getActivity()).showRight1();
                return;
            case R.id.tv_waimai:
                this.select = 1;
                this.selectIndex = 0;
                ...
                ((OrderActivity) getActivity()).showRight2();
                return;
    ...
    private void spliteList(List<OrderInfo> list) {
        ...
        for (int size = list.size() - 1; size >= 0; size--) {
            OrderInfo orderInfo = (OrderInfo) list.get(size);
